...
import type { StorySimulationKind } from "@/lib/story";
import { DeckWave } from "./deck-wave";
import { FeedSlow, HeatRace, RunawayClock } from "./sims-chemistry";
import { BudgetSplit, PriceCap, SupplyShift } from "./sims-economics";
import { ContaminationPath, MarkerMatch, SuspectFunnel } from "./sims-biology";
import { HorseHollow, SiegeClock, StoryCheck } from "./sims-history";
import { cardSoft } from "@/lib/ui";

/** Exhaustive on purpose: a new kind must fail the build, not silently
 * render somebody else's lab. */
export function StorySimulation({ kind }: { kind: StorySimulationKind }) {
  switch (kind) {
    case "timed-pushes":
      return <TimedPushes />;
    case "storm-band":
  ...
  return (
    <section className={`${cardSoft} rise overflow-hidden rounded-2xl`}>
      ...
      <div className="relative h-24 bg-white">
        ...
        <div className="pointer-events-none absolute inset-x-5 bottom-3 flex justify-between text-[12px] text-muted">
          <span>small push</span>
          <span className={together ? "font-medium text-rose" : "text-sage"}>
...
function StormBand() {
  ...
  return (
    <section className={`${cardSoft} rise rounded-2xl px-5 py-5`}>
