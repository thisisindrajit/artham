...
import { SimBar, SimFrame, SimSlider, SimToggle } from "./sim-kit";
...
const CITY = 300_000;
/** Roughly a quarter of people share any single marker value. */
const PER_MARKER = 0.25;

const pool = (markers: number) => CITY * Math.pow(PER_MARKER, markers);

export function MarkerMatch() {
  const [markers, setMarkers] = useState(6);
  const people = pool(markers);
...
const STEPS = [
  ...
  { id: "plate", label: "Loaded onto the plate", who: "Owner + attacker" },
];
    ...
    >
      <div className="space-y-3.5">
        <SimBar
          ...
          value={Math.min(searched, 300_000)}
          max={300_000}
          tone="ink"
